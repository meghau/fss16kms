...
"""
  Normalization helps to reduce magnitude effects when different quantities are represented at different scales. 
  Height can be represented in inch or cm : to a algoritm 5cm can seem more than 4 inchs while this isnt true.
  The same thing can be said about comparing time to inches, anything unit type to any other unit type. 
  Normalization allows us to makes a value unitless and allows us to compare one unit type to another more effectviely. 
"""
...
def eucledian(one, two):
  ...
  return sum([ (x - y)**2 for x,y in zip( one, two) ])**0.5 
...
def spread(obtained, ideals):
  ...
  d_sum = sum([ abs( x - d_bar ) for x in distances ])
  return (d_f + d_l + d_sum)/ ( d_f + d_l + (n-1)*d_bar )
...
def igd(obtained, ideals):
  ...
  return sum([closest(ideal,obtained)[0] for ideal in ideals]) / len(ideals)

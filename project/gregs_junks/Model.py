...
class Model( object ) :
  ...
  def generatePathsFromWaypoints( self, n, maxLen=10, showPaths=False ) : 
    """
  ...
  def example( i ) :
    m = Model()
    m.addObjective( Objective( "time",     0, 0, sys.maxint, lt ) ) 
    m.addObjective( Objective( "score",    0, 0, sys.maxint, gt ) )
    m.addObjective( Objective( "health", 100, 0, 200, gt ) ) 
    m.addObjective( Objective( "goal",     0, 0,   1, gt ) ) 

    m.addRule( Rule.add  ( "time", 1 ) ) 
    m.addRule( Rule.addIf( 3, "score", 5 ) ) 

...
from Model_rel  import Model, Objective, ColorGradient
...
class Mutator:
  ...
  def __call__( self, model, mom, dad ) : 
    ...
    while random() < self.extend_rate : 
       new.append( ( randint( -self.step_size_x, self.step_size_x ), 
                     randint( -self.step_size_y, self.step_size_y )
                 ) ) 
...
def GA( m,
  ...
  while gen < gens :
    ...
    for i in population :
      logging.info( "%s : %s"%(i.score, i.data) ) 
...
if __name__ == '__main__' : 
 ...
 # m.renderMap( 4, showGrid=False )
  ...
  step = []
  goal = []
  for iterx in xrange( 50 ) : 
    logging.info( "Running %d"%(iterx) ) 
    adjLst = m.getWaypoints( coverage=0.03, renderNetwork=False ) 
    pop    = m.generatePaths( 50, adjLst, maxLen=25, weightNodes=False, showPaths=False ) 

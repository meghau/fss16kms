...
class Path( object ) : 
  ...
  def walk( self ) :
    ...
    for p in self[1:] :
      ...
      for q in bresenhams( (cx,cy), (nx,ny) ) :
        if( q != (cx,cy) ) : yield( q )
...
class Model( object ) :
  ...
  def generatePaths( self, n, adjLst, maxLen=10, showPaths=False, weightNodes=True ) : 
    ...
    while( len( paths ) < n ) :
      ...
      while len( path ) < maxLen : 
        if weightNodes : 
           npos = weighted_choice( adjLst[cpos], weights )
        else : 
           npos = choice( adjLst[cpos] ) 

        relative = ( npos[0] - cpos[0], npos[1] - cpos[1] )
        cpos = npos
  ...
  def draw_path( self, path, color, drawWaypoints=True, tag="overlay" ) :
    ...
    for rpos in path[1:] :
      npos = (cpos[0] + rpos[0], cpos[1] + rpos[1])
      self.draw_line( cpos, npos, color, tag=tag )
      cpos = npos
